...
def tanka_prompt(configs,
    ...
    if (critic_stance == 0):  # stanceあり
        sys = "あなたは短歌の表現や内容を詳細に評価することのできる役立つアシスタントです。"
    else:                     # stanceなし
        sys = """あなたは短歌の表現や内容を詳細に評価することのできる役立つアシスタントで、以下のような価値観を持っています。
{stance}
...
お題が「{odai}」であることを踏まえて、この短歌の表現や内容についての鑑賞文を出力してください。""".format(odai = Theme, author = Author, tanka = Tanka)
...
お題が「{odai}」であることと作者のコメント、そしてこの指摘を踏まえて、この短歌の表現や内容についての鑑賞文を出力してください。""".format(odai = Theme, author = Author, tanka = Tanka, author_comment = Author_comment, human = Human_comment)
...
以下に、{row["Author"]}さんの短歌「{row["Content"]}」について、{str(len(LLMs))}つの異なるLLMが生成したコメントがあります。
"""
        LLM_num = 1
        for LLM in LLMs:
            LLM_name = LLM.replace("LLM:", "")
            row[f"{LLM}"] = row[f"{LLM}"].replace("\n\n", "\n")
            user = user + f"\n{str(LLM_num)}. {LLM_name}のコメント: \n"
            user = user + "```txt\n" + row[f"{LLM}"] + "\n```\n"
...
1. 評価者間の共通点
2. 評価者間の相違点
最後に、この短歌についての総評を補足してください。"""

        assist = f"""わかりました。以下に{str(len(LLMs))}個のコメントの要約と、ファシリテーターとしての総評を出力します。
"""
    # 題詠
    else:
        sys = f"""あなたは、異なる評価者の複数のコメントについて要約、整理することのできるファシリテーターです。
以下に、{theme}というお題で{row["Author"]}さんが詠んだ短歌「{row["Content"]}」について、{str(len(LLMs))}つの異なるLLMが生成したコメントがあります。
"""
        LLM_num = 1
        for LLM in LLMs:
            LLM_name = LLM.replace("LLM:", "")
