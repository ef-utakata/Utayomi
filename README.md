...
* 2025年8月7日: 1.3.2 公開
    * **再現性レポート機能**: 選評Markdownに詳細な実行環境・設定情報を自動付記
      - リポジトリURL、コミットハッシュ、実行コマンドを記録
      - 完全な再現手順を提供し、評価結果の信頼性を向上
    * **TTS読み上げ改善**: 短歌の正式な読み方（2回読み上げ）をTTS原稿に反映
      - 単作・連作両方のテンプレートで短歌を2回読み上げる仕様に変更
      - より自然で正統な短歌朗読を実現
